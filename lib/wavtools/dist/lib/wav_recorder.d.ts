...
export class WavRecorder {
   ...
   * @private
   * @param {string} name
   * @param {{[key: string]: never}} data
   * @param {AudioWorkletNode} [_processor]
   * @returns {Promise<{[key: string]: never}>}
   */
  private _event;
  /**
   ...
   * @returns {true}
   */
  listenForDeviceChange(callback: never): true;
  /**
   ...
   * @param {(data: { mono: Int16Array; raw: Int16Array }) => never} [chunkProcessor]
   * @param {number} [chunkSize] chunkProcessor will not be triggered until this size threshold met in mono audio
   * @returns {Promise<true>}
   */
  record(
    chunkProcessor?: (data: { mono: Int16Array; raw: Int16Array }) => never,
    chunkSize?: number,
  ): Promise<true>;
